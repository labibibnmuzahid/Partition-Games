// generate_tables_corner_5row.js
...
const MAX_R1 = 10; // The largest value for the first row
const MAX_R2 = 10; // The largest value for the second row
const MAX_R3 = 10; // The largest value for the third row
const MAX_R4 = 10; // The largest value for the fourth row
const MAX_R5 = 10; // The largest value for the fifth row
const OUTPUT_FILE = "5_row_analysis.csv";

// 3. --- CORE LOGIC: The main function to generate the data ---
async function generateAnalysisTable() {
  console.log("🚀 Starting 5-row analysis table generation...");
  const results = [];
  ...
  let count = 0;
  // Iterate through all 5-row partitions (r1 >= r2 >= r3 >= r4 >= r5)
  for (let r1 = 1; r1 <= MAX_R1; r1++) {
    for (let r2 = 1; r2 <= Math.min(r1, MAX_R2); r2++) {
      for (let r3 = 1; r3 <= Math.min(r2, MAX_R3); r3++) {
        for (let r4 = 1; r4 <= Math.min(r3, MAX_R4); r4++) {
          for (let r5 = 1; r5 <= Math.min(r4, MAX_R5); r5++) {
            const partition = [r1, r2, r3, r4, r5];
            const positionTuple = JSON.stringify(partition);
            const board = new Board(partition);

            count++;
            if (count % 100 === 0) {
              console.log(`Analyzed ${count} partitions... Current: [${r1}, ${r2}, ${r3}, ${r4}, ${r5}]`);
            }
  ...
  console.log(
    `Generated data for ${count} partitions up to [${MAX_R1}, ${MAX_R2}, ${MAX_R3}, ${MAX_R4}, ${MAX_R5}] in ${duration.toFixed(
      2
    )} seconds.`
  );
}
